// Helper functions (inline for simplicity)
  function parseEventDate(dateStr) {
    ...
    const date = new Date(Date.UTC(2025, months[month], dayNum));
    return date;
  }

  function enhanceEventData(events) {
    if (!Array.isArray(events)) {
      console.warn('enhanceEventData: events is not an array:', events);
      return [];
    }
    ...
    return events.map(event => {
      if (!event || typeof event !== 'object') {
        ...
        return {
          ...
          series: 'Unknown',
          start: new Date(),
          displaySeries: 'Unknown'
        };
      }
      
      const displaySeries = (event.series && event.series.includes("NASCAR")) ? "NASCAR" : (event.series || "Unknown");
  ...
  // Simple calendar download functions
  function downloadICS(event) {
    alert('Calendar download feature coming soon!');
  }
  
  function downloadSeriesICS(series) {
    alert('Series calendar download feature coming soon!');
  }
    ...
    events.forEach(event => {
      ...
      if (Date.now() >= eventStartDate.getTime() + 24 * 60 * 60 * 1000) {
        ...
        if (event.series === "F1") {
          ...
          const result = window.resultsData.f1Past.results.find(r => 
          ...
          const result = window.resultsData.indyPast.results.find(r => 
          ...
          if (event.race.includes("Duel")) {
            results = window.resultsData.nascarPast.results.filter(r => 
              r.race.toLowerCase().includes("duel")
            );
          } else {
            const eventShortName = event.race.split(" at ")[0].toLowerCase();
            const result = window.resultsData.nascarPast.results.find(r => 
  ...
  function displayUpcomingEvent() {
    ...
    if (!window.scheduleData || !window.scheduleData.f1Events || !window.scheduleData.indyEvents || !window.scheduleData.nascarEvents) {
      console.warn('Schedule data not available');
      return;
    }
    
    const allEvents = enhanceEventData(window.scheduleData.f1Events.concat(window.scheduleData.indyEvents, window.scheduleData.nascarEvents));
    ...
    async function createEventCard(event) {
      ...
      card.addEventListener('click', () => {
        ...
        setTimeout(() => {
          // generateModalAIPrediction(); // Commented out as aiPredictions is not defined
        }, 500);
      });
  ...
  function createMobileCard(event) {
    ...
    // Safely handle the date attribute
    const eventDate = event.start || new Date();
    card.setAttribute('data-date', eventDate.toISOString());
  ...
  function createTableRow(event, includeSeriesBadge = false) {
    ...
    // Safely handle the date attribute
    const eventDate = event.start || new Date();
    row.setAttribute('data-date', eventDate.toISOString());
    const localTime = convertUTCtoLocal(event.date, event.time_utc);
    const nowRow = new Date();
  ...
  function populateSchedules() {
    // Check if data is available
    if (!window.scheduleData || !window.scheduleData.f1Events || !window.scheduleData.indyEvents || !window.scheduleData.nascarEvents) {
      console.error('Schedule data not available for populateSchedules');
      return;
    }
    ...
    if (!f1Table || !indyTable || !nascarTable || !combinedTable) {
    ...
    window.scheduleData.f1Events.forEach(event => {
    ...
    window.scheduleData.indyEvents.forEach(event => {
    ...
    const allNascarEvents = [...window.scheduleData.nascarEvents]; // Use the single nascarEvents array
  ...
  async function generateAIPredictions() {
    ...
    try {
      ...
      const upcomingEvents = allEvents.filter(event => {
        ...
        return event.start > now;
           ...
           // const predictions = await aiPredictions.getBatchPredictions(upcomingEvents); // Commented out as aiPredictions is not defined
        
                 // Display analysis
         let html = '';
         // predictions.forEach(({ event, prediction, success, error }) => { // Commented out as aiPredictions is not defined
         //   if (success) {
    ...
    async function generateModalAIPrediction() {
      ...
      try {
        ...
        // const prediction = await aiPredictions.getRacePrediction(event, weatherData); // Commented out as aiPredictions is not defined
        // const predictionsHTML = aiPredictions.generateCompactPredictionsHTML(event); // Commented out as aiPredictions is not defined
    ...
    const modalDownloadBtn = document.getElementById('modalDownloadICS');
    if (modalDownloadBtn) {
